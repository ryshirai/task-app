...
pub async fn register(
    ...
    let org = sqlx::query_as::<_, (i32,)>("INSERT INTO organizations (name) VALUES ($1) RETURNING id")
...
pub async fn forgot_password(
    ...
    println!("PASSWORD RESET TOKEN for {}: {}", user.username.unwrap_or_default(), token);
...
pub async fn reset_password(
    ...
    let reset = sqlx::query_as::<_, (i32, i32)>("SELECT id, user_id FROM password_resets WHERE token = $1 AND expires_at > $2")
        .bind(&input.token)
        .bind(Utc::now())
        .fetch_optional(&state.pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or((StatusCode::NOT_FOUND, "Invalid or expired reset token".to_string()))?;
